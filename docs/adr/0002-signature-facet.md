# ADR-0002：签名 facet（ApiKey 聚合）

- **状态**：Accepted
- **日期**：2026-07-29
- **关联**：ADR-0001（聚合边界与标识模型）；架构仓库 `app-registry.md`、`architecture.md` §4/§6.1；本项目 `CONTEXT.md`「签名 facet」「bootstrap 端点加固」

## 背景（Context）

签名 facet 供 `cartisan-openapi` 机机验签（HMAC-SHA256）：消费方应用持 `apiKey`/`apiSecret` 签自己的请求；provider 服务的 `RemoteApiKeyProvider`（Caffeine 30min + HTTP）调本服务 `/by-appId` 取 `ApiKeyInfo` 来验签。

约束：
- 架构不变式：签名 `apiSecret` 必须**可取回**（消费方要明文做 HMAC）；`/by-appId` 只返签名 facet；bootstrap 端点无验签、必须加固。
...
2. **`api_secret` = AES-GCM 密文**：主密钥来自环境变量；DB 只存密文；`LocalApiKeyProvider` 在内存中解密得明文，组进 `ApiKeyInfo.apiSecret` 供验签。
   - 加密是**可逆**的（与 hash 不同）——这正是签名 facet 的需要：验签要明文做 HMAC，故必须能取回。明文仅存在于内存 + 加密通道，不入库、不入日志。

3. **`api_key` 用 SecureRandom 生成**（修旧 hutool `RandomUtil` 不安全实现）；全局唯一（含软删行）；可完全轮换。

4. **不设 `api_secret_prev`**：零停机轮换重叠先不做（YAGNI，当前无零停机轮换需求）；真需要再加影子列（与 SSO `client_secret_prev_hash` 同款模式）。
...
8. **`/by-appId` = `GET /api/app-registry/api-keys/{apiKey}`**：返 `ApiResponse.ok(ApiKeyInfo)`；路径变量与 `RemoteApiKeyProvider` 契约对齐；**只返签名 facet，不返 SSO 字段**。
...
- 正向：签名 facet 与 SSO facet 解耦；`apiSecret` 可取回供验签、又非明文入库；与 `cartisan-openapi` 契约（4 字段 `ApiKeyInfo`）对齐；清掉旧明文 secret + 不安全随机两个安全债。
...
- **`apiSecret` hash 存**：否。验签要明文做 HMAC，hash 不可逆，无法取回——与签名 facet 的"可取回"不变式冲突（这是 SSO facet 才用 hash 的原因）。
- **`api_secret_prev` 影子列（零停机轮换）**：未选。当前无零停机轮换需求，YAGNI；留作未来。
- **`permissions` 列（per-key ACL）**：否。框架已判定臆想、已硬删；签名=纯认证。
- **bootstrap token / mTLS 加固 `/by-appId`**：否。app-registry 只对内可达、外部不知此服务，网络边界已是强制信任边界；端点再叠鉴权是冗余（与用户"不过度设计"约束一致）。
